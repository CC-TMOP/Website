#! /bin/bash
JS_PATH=~/yueyuea/Website/demo/static/js/
JS_PATH_DIST=${JS_PATH}dist/
JS_PATH_SRC_INDEX=${JS_PATH}src/Index/
JS_PATH_SRC_MOP=${JS_PATH}src/Mop/

CSS_PATH=~/yueyuea/Website/demo/static/css/
CSS_PATH_DIST=${CSS_PATH}dist/
CSS_PATH_SRC=${CSS_PATH}src/

find $JS_PATH_SRC_INDEX -type f -name '*.js' | sort | xargs cat > ${JS_PATH_DIST}main.js
